...
import { Clock3, Flame, History, PlayCircle, Sparkles } from "lucide-react";
import HistoryVideoMenu from "@/app/components/HistoryVideoMenu";
...
export default async function HistoryPage() {
      ...
      <div className="relative min-h-[calc(100vh-73px)] overflow-hidden bg-[#070707] px-4 py-8 text-white sm:px-6 lg:px-10">
        ...
        <div className="relative mx-auto max-w-5xl rounded-3xl border border-white/10 bg-zinc-950/70 p-8 text-center backdrop-blur-xl">
          ...
          <p className="mx-auto mt-3 max-w-xl text-zinc-400">
            Your watched videos are tracked only for your account, so log in
            first to view and continue where you left off.
          </p>
  ...
  const watchedToday = history.filter((item) => {
  ...
  const uniqueChannels = new Set(history.map((item) => item.video.channelId))
    .size;

  const userName =
    currentUser.firstName || currentUser.name?.split(" ")[0] || "Creator";
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-zinc-300 sm:text-base">
                Revisit your recent videos, pick up where you stopped, and keep
                your watch streak alive.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:min-w-[500px]">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-2xl font-extrabold">{uniqueChannels}</p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-red-500/20 via-orange-400/10 to-transparent p-4">
                ...
                <p className="mt-2 text-2xl font-extrabold">{watchedToday}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        {history.length === 0 ? (
          <div className="mt-8 rounded-3xl border border-dashed border-white/20 bg-zinc-950/60 p-12 text-center">
            ...
            <p className="mx-auto mt-2 max-w-xl text-zinc-400">
              Start watching videos and they will show up here automatically.
            </p>
